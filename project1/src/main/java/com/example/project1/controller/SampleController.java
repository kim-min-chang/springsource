...
public class SampleController {
    ...
    // 1) rttr.addAttributes("이름", 값) : 경로 ? 다음에 따라가는 값=> ${param.이름}
    // 2) rttr.addFlashattributes("이름",값) : 세션을 이용하기 때문에 따라가는건 안보임=>${이름}
    ...
    public String basic2(RedirectAttributes rttr) {
        ...
        rttr.addAttribute("age", 15); // redirect 시 주소의 파라메터로 딸려보내기
        rttr.addAttribute("name", "hong"); // redirect 시 주소의 파라메터로 딸려보내기
        rttr.addFlashAttribute("addr", "seoul");
    ...
    public void postCalc2(CalcDto calc, Model model) {
        log.info("입력값 가져오기");
        log.info("{} + {} = {}", calc.getNum1(), calc.getOp(), calc.getNum2());
