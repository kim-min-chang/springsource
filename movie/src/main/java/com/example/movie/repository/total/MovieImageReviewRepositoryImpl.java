...
@Log4j2
public class MovieImageReviewRepositoryImpl extends QuerydslRepositorySupport implements MovieImageReviewRepaisty {
    ...
    public Page<Object[]> getTotalList(String type, String keyword, Pageable pageable) {
        ...
        JPQLQuery<Long> inum = JPAExpressions.select(movieImage.inum.max()).from(movieImage)
                .groupBy(review.movie.eq(movieImage.movie));
